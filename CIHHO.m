...
%%
function [Rabbit_Energy,Rabbit_Location,CNVG,Div,Xpl2,Xpt2]=CIHHO(N,T,lb,ub,dim,fobj)
...
while t<T
    ...
    es=rand()/2;
    if t<es*T
       E1=1+(1-(t/(es*T)).^2).^(1/2); % factor to show the decreaing energy of rabbit
    elseif t>=es*T
       E1=1-(1-((t-T)/(T*(1-es))).^2).^(1/2);
    end
    for i=1:size(X,1)
        E0=2*rand()-1; %-1<E0<1
        Escaping_Energy=E0*E1;  % escaping energy of rabbit
...
%% phase 2: performing team rapid dives (leapfrog movements)
            if r<0.5 && abs(Escaping_Energy)>=0.5, % Soft besiege % rabbit try to escape by many zigzag deceptive motions
                ...
                if fobj(X1)<fobj(X(i,:)) % improved move?
                    ...
                    X2=(RR-Escaping_Energy*abs(Jump_strength*RR-X(i,:))+rand(1,dim).*Levy(dim)*abs(cos(t)));
            ...
            if r<0.5 && abs(Escaping_Energy)<0.5, % Hard besiege % rabbit try to escape by many zigzag deceptive motions
               ...
               Jump_strength=(((t/(T*pi)).^2+1).^(-1));
                ...
                if fobj(X1)<fobj(X(i,:)) % improved move?
                    ...
                    X2=(RR-Escaping_Energy*abs(Jump_strength*RR-mean(X))+rand(1,dim).*Levy(dim)*abs(cos(t)));
                    if (fobj(X2)<fobj(X(i,:))), % improved move?
                        X(i,:)=X2;
                    end
                end
